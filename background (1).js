...
async function startBulkListing(data) {
  ...
  const total = parseInt(data.numListings) || 1;
  state = {
    activeJob: true,
    totalToCreate: total,
    createdCount: 0,
  ...
  // rotation directly from "draftListing" + "customLocations" using its
  // own #idx= hash — save it once here so every tab is self-sufficient.
  await chrome.storage.local.set({ draftListing: data });

  await openAllListingTabs(total);
}

// ============================================================
// PARALLEL FIRE-AND-FORGET TAB OPENING (Phase 1)
// ------------------------------------------------------------
// All listing tabs are opened up front (small natural stagger
// between each). Only the FIRST tab is made active — every other
// tab stays in the background for its entire lifetime and is
// NEVER activated or focused. Each tab's content script is fully
// self-sufficient (reads its own data via the #idx= hash and
// fills itself immediately on load) — there is no handshake, no
// queue, and no dependency on background.js hearing back from a
// tab before the next one can proceed. Nothing here requires the
// browser window to be watched or focused.
// ============================================================
async function openAllListingTabs(total) {
  for (let i = 0; i < total; i++) {
    if (!state.activeJob) return;

    const isFirst = (i === 0);
    const url = `https://www.facebook.com/marketplace/create/item#idx=${i}`;
    ...
    updateProgress(
      `Opened ${i + 1}/${total} tabs...`,
      (i / total) * 100
    );

    if (i < total - 1) {
      // Natural stagger only — not a coordination wait for anything.
      const delay = 1000 + Math.floor(Math.random() * 800); // 1.0–1.8s
      await sleep(delay);
    }
  }
}
...
async function handleDraftSaved(tabId) {
  ...
  state.createdCount++;
  console.log(`[Lister Logs] [BG] Draft saved (tab ${tabId}). Total: ${state.createdCount}/${state.totalToCreate}`);
  await saveState();
  ...
  // This tab's job is done — close it so tabs don't pile up.
  try { await chrome.tabs.remove(tabId); } catch (e) {}
  state.openTabs = (state.openTabs || []).filter(id => id !== tabId);
  await saveState();
...
chrome.tabs.onRemoved.addListener(async (tabId, removeInfo) => {
  ...
  if (state.activeJob && state.openTabs && state.openTabs.includes(tabId)) {
    console.log("[BG] A listing tab was closed manually before finishing:", tabId);
    state.openTabs = state.openTabs.filter(id => id !== tabId);
    await saveState();
  }
});
