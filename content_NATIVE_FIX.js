...
async function selectDropdownOption(dropdownEl, optionText) {
  ...
  // Step 1: Open dropdown using native click (bypasses React synthetic event system)
  const nativeClick = (element) => {
    const rect = element.getBoundingClientRect();
    const x = rect.left + rect.width / 2;
    const y = rect.top + rect.height / 2;

    // Native mouse events (not synthetic - React listens to these)
  ...
  nativeClick(dropdownEl);
  await sleep(2000); // Wait for dropdown to FULLY render

  // Step 2: Find option - try multiple strategies
  let optionEl = null;

  // Strategy 1: role="option" with exact match
  ...
  if (!optionEl) {
    optionEl = allOptions.find(el => {
      const text = el.textContent.trim().toLowerCase();
      return text.includes(optionText.toLowerCase()) && text.length < 50;
    });
  }

  // Strategy 3: Any visible element with exact text
  if (!optionEl) {
    optionEl = [...document.querySelectorAll('div, span, li')].find(el => {
      const text = el.textContent.trim().toLowerCase();
      return el.children.length === 0 && 
             text === optionText.toLowerCase() &&
             el.offsetParent !== null;
    });
  }

  // Strategy 4: Partial text match in any element
  if (!optionEl) {
    optionEl = [...document.querySelectorAll('div, span, li')].find(el => {
      const text = el.textContent.trim().toLowerCase();
      return el.children.length === 0 && 
  ...
  console.log("[DROPDOWN] Found:", optionEl.textContent.trim());

  // Step 3: Click option using NATIVE events (crucial for React)
  optionEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
  await sleep(600);

  nativeClick(optionEl);
  await sleep(800);
...
async function setLocation(location) {
    ...
    if (!selected) {
      ...
      const allElements = [...document.querySelectorAll('div, span, li')].filter(el => {
        ...
        return el.children.length === 0 && 
               ...
               (text.includes(locationCity) || text.includes('new york')) &&
               el.offsetParent !== null;
      });
  ...
  if (!selected) {
    console.log("[LOCATION] Using keyboard fallback...");
