...
async function runPhase1(data) {
  ...
  try {
    ...
    // Find SPAN with "Condition" text, then get parent DIV
    const conditionSpan = [...document.querySelectorAll('span')]
      .find(el => el.children.length === 0 && el.textContent.trim() === 'Condition');
    const conditionDrop = conditionSpan ? conditionSpan.parentElement : 
                          (findFieldByLabel("Condition", "div") || 
...
async function setLocation(location) {
  ...
  // CRITICAL: Wait for Facebook to fetch suggestions
  await sleep(4000);

  // Step 4: SCROLL THE DROPDOWN CONTAINER TO TOP (so first option is visible)
  // Find the dropdown container (usually role="listbox" or a scrollable div)
  let dropdownContainer = document.querySelector('[role="listbox"]') ||
                          document.querySelector('div[style*="overflow"]') ||
                          document.querySelector('ul');

  if (dropdownContainer) {
    console.log("[LOCATION] Scrolling dropdown container to top...");
    dropdownContainer.scrollTop = 0; // Scroll to very top
    await sleep(500);
  }

  // Step 5: Find first option and ensure it's visible
  let firstOption = null;

  // Try role="option" first
  const roleOptions = [...document.querySelectorAll('[role="option"]')];
  if (roleOptions.length > 0) {
    firstOption = roleOptions[0];
    console.log("[LOCATION] Found option:", firstOption.textContent.trim());
  }

  // If not found, try any clickable item in dropdown
  if (!firstOption && dropdownContainer) {
    const items = dropdownContainer.querySelectorAll('div, span, li');
    for (const item of items) {
      if (item.textContent.trim().length > 2 && item.offsetParent !== null) {
        firstOption = item;
        console.log("[LOCATION] Found container item:", item.textContent.trim());
        break;
      }
    }
  }

  // Step 6: Click the option (if found and visible)
  if (firstOption) {
    ...
    if (!isVisible) {
      console.log("[LOCATION] Option not visible, scrolling into view...");
      firstOption.scrollIntoView({ behavior: 'instant', block: 'nearest' });
      await sleep(500);
    }

    // Click with native events
    ...
    clickOption(firstOption);
    await sleep(800);

    // If click didn't work, try keyboard
    const afterClick = document.querySelector('input[aria-label="Location"]');
    if (afterClick && afterClick.value !== location.split(',')[0]) {
      console.log("[LOCATION] Click didn't work, using keyboard...");
      ...
      console.log("[LOCATION] Used keyboard selection");
    } else {
      console.log("[LOCATION] Click successful!");
    }
  } else {
    // No option found at all - use keyboard only
    console.log("[LOCATION] No option found, using keyboard fallback...");
